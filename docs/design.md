...
- V1 default client renders:
  ...
  - Character creation's own interactive dice reveal (`client.ability_score_rolls`, a distinct message family from `client.roll*` above): the 4d6-drop-lowest ability-score method reveals all six rolled sets before asking the player to assign any of them — a private, single-player exchange with no spectator to hide a result from, so unlike `client.roll*` there's no reveal-relay round trip, only a single acknowledgment once the player has seen everything. The assignment questions that follow ask by ability ("Assign which score to Strength?", offering the remaining rolled totals) rather than by score, so the player always sees the whole pool.
