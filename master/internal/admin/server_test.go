...
func newTestServer(t *testing.T, restartRequested chan struct{}) (*admin.Server, *httptest.Server) {
	...
	srv := admin.New(logger, s, s, s, s, "", "127.0.0.1:8090", seed, restartRequested, nil, "", "", "", nil, session.NewHub())
...
func newTestServerWithLLM(t *testing.T, llmProvider llm.Provider) (*admin.Server, *httptest.Server) {
	...
	logger := slog.New(slog.NewTextHandler(io.Discard, nil))
	s := newTestStore(t)
	srv := admin.New(logger, s, s, s, s, "", "127.0.0.1:8090", nil, nil, llmProvider, "test-model", t.TempDir(), "", nil, session.NewHub())
	httpSrv := httptest.NewServer(srv.Handler())
	t.Cleanup(httpSrv.Close)
	return srv, httpSrv
}
