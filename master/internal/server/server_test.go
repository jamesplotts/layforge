...
func TestHandleWebSocket_ValidConnect_PersistsHandshakeEvents(t *testing.T) {
	...
	srv := server.New(logger, events, nil, "", nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, session.NewHub())
...
func newTestServerWithHistory(t *testing.T) *httptest.Server {
	...
	t.Cleanup(func() { _ = events.Close() })
	return httptest.NewServer(server.New(logger, events, nil, "", nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, session.NewHub()).Handler())
}
...
func TestServe_LogHistoryRequest_PersistenceDisabled_RespondsWithError(t *testing.T) {
	ts := newTestServer(t) // uses server.New(logger, nil, nil, "", nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, hub) — no store
	defer ts.Close()
...
func TestServe_NarrativePlayerInput_RendersAndBroadcastsBubble(t *testing.T) {
	...
	logger := slog.New(slog.NewTextHandler(io.Discard, nil))
	ts := httptest.NewServer(server.New(logger, nil, fake, "test-model", nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, session.NewHub()).Handler())
	defer ts.Close()
...
func TestHandleWebSocket_AuthProvider_CorrectPassword_Joins(t *testing.T) {
	...
	ts := httptest.NewServer(server.New(logger, nil, nil, "", authProvider, nil, nil, nil, nil, nil, nil, nil, nil, nil, session.NewHub()).Handler())
	defer ts.Close()
...
func newTestServer(t *testing.T) *httptest.Server {
	t.Helper()
	logger := slog.New(slog.NewTextHandler(io.Discard, nil))
	srv := server.New(logger, nil, nil, "", nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, session.NewHub())
