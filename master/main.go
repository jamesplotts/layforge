...
// canceled (SIGINT/SIGTERM) or the listener fails, then shuts down
// gracefully. Split out from main so the startup/shutdown logic is
// callable from a test without invoking os.Exit.
func run(addr string, logger *slog.Logger) error {
	mux := http.NewServeMux()
	mux.Handle("/ws", server.New(logger).Handler())
